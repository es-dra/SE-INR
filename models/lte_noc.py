"""
LTE-NoC: LTE without Cell-conditioned phase (h_p(c)).

Removes the phase conditioning on cell size to test whether
removing c-dependence improves OOD generalization.

Changes from LTE:
- Removed self.phase = nn.Linear(2, hidden_dim//2, bias=False)
- Removed q_freq += self.phase(rel_cell...) line
- Phase is fixed at 0 (no cell conditioning)
- Fixed hardcoded .cuda() to use .to(device)
"""
...
@register('lte-noc')
class LTENoC(nn.Module):
    """
    LTE variant without h_p(c) phase conditioning.

    In LTE: q_freq += phase(cell) -> phase depends on cell size
    In LTE-NoC: q_freq has no phase offset -> phase = 0
    """
    ...
    def __init__(self, encoder_spec, imnet_spec=None, hidden_dim=256,
        ...
        # NOTE: No phase module - phase is fixed at 0
    ...
    def gen_feat(self, inp):
        self.inp = inp
        # Fixed: use device-aware coordinate creation instead of hardcoded .cuda()
        device = inp.device
    ...
    def query_rgb(self, coord, cell=None):
        ...
        for vx in vx_lst:
            for vy in vy_lst:
                ...
                # NOTE: cell is still used for upinput interpolation, but NOT for phase
                # basis generation - phase is FIXED at 0 (no cell conditioning)
                bs, q = coord.shape[:2]
                ...
                # REMOVED: q_freq += self.phase(rel_cell.view((bs * q, -1))).view(bs, q, -1)
                # Phase is now fixed at 0 - no cell conditioning
