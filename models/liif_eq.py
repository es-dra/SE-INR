...
class LIIFEQ(nn.Module):
    ...
    def __init__(self, encoder_spec, imnet_spec=None,
        ...
        if imnet_spec is not None:
            ...
            if self.feat_unfold:
                if tranNum > 1:
                    ...
                    self.adjust = fn.Fconv_PCA(
                        kernel_size,
                        imnet_in_dim // tranNum,
                        imnet_in_dim // tranNum * 9,
                        tranNum,
                        inP=kernel_size,
            ...
            self.imnet = models.make(
                imnet_spec,
                args={'tranNum': tranNum, 'in_dim': imnet_in_dim},
            )
        else:
            self.imnet = None
        ...
        for vx in vx_lst:
            for vy in vy_lst:
                ...
                if self.cell_decode:
                    ...
                    rel_cell_x = rel_cell[:, :, 0].unsqueeze(2).repeat([1, 1, self.tranNum])
                    rel_cell_y = rel_cell[:, :, 1].unsqueeze(2).repeat([1, 1, self.tranNum])
                    q_feat = torch.cat([q_feat, rel_cell_x, rel_cell_y], dim=-1)
    ...
    def ImfunctionObserve(self, ind, shave=10):
        ...
        inp = q_feat.unsqueeze(3).unsqueeze(3) * x.unsqueeze(0).unsqueeze(0).unsqueeze(0)
    ...
    def coordGen(self, Num=60):
        if not hasattr(self, '_coord_cache') or self._coord_cache[0] != Num:
            ...
            x = torch.Tensor(x)
            y = x.permute(1, 0)
            coord = torch.stack([x, y], dim=2)
            self._coord_cache = (Num, coord)
