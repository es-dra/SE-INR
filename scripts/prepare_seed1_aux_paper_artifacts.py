...
def plot_scale_gain(out_dir: Path, quality_per_scale: pd.DataFrame) -> None:
    plot_models = ["LIIF", "LIIF-EQ", "LTE-EQ", "LTE-NoCell", "LTE-FeaturePhase", "SC-INR-Fixed", "SC-INR-Adaptive"]
    ...
    for ax, dataset in zip(axes, ["bsd100", "urban100"]):
        for model in plot_models:
            ...
            ax.plot(
                ...
                marker="o",
                linewidth=2.0 if model == "SC-INR-Adaptive" else 1.3,
    ...
    for ax, dataset in zip(axes, ["bsd100", "urban100"]):
        ...
        for _, row in sub.iterrows():
            ...
            ax.scatter(
                ...
                s=68 if model == "SC-INR-Adaptive" else 42,
                color=STYLE.get(model, "#888888"),
                edgecolor="black" if model == "SC-INR-Adaptive" else "none",
                linewidth=0.8,
                zorder=3,
            )
...
def write_key_findings(out_dir: Path, quality_split: pd.DataFrame, consistency_agg: pd.DataFrame) -> None:
    ...
    lines.extend([
        ...
        "- Use `fig_quality_consistency_tradeoff.pdf` to show that SC-INR-Adaptive improves consistency while preserving quality better than purely removing cell response.",
        "",
    ])
