...
> SC-INR-Adaptive 在 OOD scale 上相对 LTE 有小幅但稳定的质量提升，同时显著改善 same-LR cross-scale observation consistency。
...
- BSD100 OOD：SC-INR-Adaptive 相对 LTE 的 PSNR-Y 为 `+0.056 dB`，SSIM-Y 为 `+0.00191`。
- Urban100 OOD：SC-INR-Adaptive 相对 LTE 的 PSNR-Y 为 `+0.052 dB`，SSIM-Y 为 `+0.00101`。
- BSD100 consistency：SC-INR-Adaptive 相对 LTE 的 SC-PSNR 为 `+8.61 dB`，texture consistency RMSE 为 `-0.00590`。
- Urban100 consistency：SC-INR-Adaptive 相对 LTE 的 SC-PSNR 为 `+7.77 dB`，texture consistency RMSE 为 `-0.01047`。
...
- `key_findings.md`：英文简要结论，后续建议由本文档替代。
- `reviewer_advice_analysis.md`：英文审阅建议分析，后续应逐步整理进中文研究笔记。
...
- `fig_scale_gain_vs_lte.pdf`：可改成少模型曲线，只画 LTE、LTE-EQ、SC-INR-Fixed、SC-INR-Adaptive。
...
- 右侧只放少量关键方法：`GT`、`Bicubic`、`LTE`、`LTE-EQ`、`SC-INR-Fixed`、`SC-INR-Adaptive`。
