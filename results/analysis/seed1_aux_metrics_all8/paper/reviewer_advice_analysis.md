...
observation consistency improves sharply for SC-INR-Adaptive.

The most important caveat is that the current evidence still does not isolate
the sinc response as the sole causal factor. SC-INR-Adaptive changes both the
...
- BSD100 OOD PSNR: SC-INR-Adaptive is +0.056 dB over LTE.
- Urban100 OOD PSNR: SC-INR-Adaptive is +0.052 dB over LTE.
...
- If w/o sinc matches SC-INR-Adaptive in PSNR but loses consistency, the paper
