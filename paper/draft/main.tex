...
observation consistency in seed1 diagnostics. A feature-conditioned phase
variant is a promising final candidate, improving seed1 PSNR and selected
qualitative examples, but still requires auxiliary metrics, a w/o-sinc ablation,
and additional seeds before being claimed as the final model.
\end{abstract}
...
final-candidate SC-INR corresponds to raw key ``SC-INR+PhiZ''.
...
\begin{table}[H]
\centering
\caption{Seed1 benchmark summary. The final-candidate SC-INR is a promising
single-seed result; multi-seed confirmation is not yet complete.}
\label{tab:seed1}
\begin{tabular}{lrrr}
\toprule
...
supports the feature-conditioned phase as a useful candidate, but not yet as a
final multi-seed conclusion.
...
SC-INR has only seed1 benchmark evidence. Second, auxiliary consistency and
texture metrics currently cover SC-INR-NoPhi, not the final-candidate variant.
Third, a w/o-sinc ablation is required before attributing the gain causally to
the analytic response. Finally, PSNR does not fully measure perceptual texture
quality; qualitative crops and texture-sensitive metrics must remain part of the
evidence package.
...
consistency, while feature-conditioned phase is a promising final-candidate
extension. The next critical step is to close the evidence gap with auxiliary
metrics, w/o-sinc ablation, and additional seeds for the final-candidate SC-INR.
